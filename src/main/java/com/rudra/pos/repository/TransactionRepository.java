...
/**
 * Persists sales relationally across two files: the header in
 * {@code transactions.csv} and the sold lines in {@code transaction_lines.csv},
 * joined on transaction id. The child table is rewritten atomically whenever
 * the parent table is persisted.
 *
 * <p>Note: the lines {@link CsvFile} is created lazily inside the
 * {@code onLoaded}/{@code onPersist} hooks rather than as a field, because
 * those hooks run from the superclass constructor — before subclass field
 * initialisers would execute.</p>
 */
public class TransactionRepository extends AbstractCsvRepository<Transaction> {
